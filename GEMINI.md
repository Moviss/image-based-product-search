...
Upload a furniture image, get ranked matches from a ~2,500 product MongoDB catalog via Vision analysis and re-ranking.

## Project Documentation

- `.ai/prd.md` — PRD with functional requirements (RF-001–RF-033) and 24 user stories
- `.ai/tech-stack.md` — technology choices and rationale
- `.ai/task-description.md` — original task specification
...
- **Mongoose 9** for local MongoDB Docker instance
- **@anthropic-ai/sdk** for Claude API (Vision + Text) used for AI logic
...
data/             — DB initialization scripts and JSON data dumps
```

## Implementation Progress

- **Steps 1-10** [DONE] — Base app using Anthropic logic, UI, Admin Panel, and Re-ranking logic implemented.
- **Step 11** [DONE] — Moved MongoDB to local Docker instance for read-only replica isolation.
...
- **MongoDB** — queries from local Docker instance (`mongodb://localhost:27017/product_search`).
- **In-memory state** for admin config and feedback — acceptable for MVP, lost on restart
- **No `"use client"` unless necessary** — prefer Server Components by default

## Next.js Docs (auto-generated, do not edit)
